...
# 全局页面配置（仅需调用一次，放在最上方）
st.set_page_config(page_title="缝合页面", page_icon="📚", layout="wide")

# ---------------------- 核心CSS样式（移除侧边栏圆圈+鼠标悬浮变色） ----------------------
st.markdown("""
    <style>
    /* 1. 完全移除侧边栏radio组件的原生圆圈（选中/未选中状态均隐藏） */
    .stRadio [role="radiogroup"] label > div:first-child {
        display: none !important;
    }

    /* 2. 侧边栏选项容器样式：统一布局，调整间距 */
    .stRadio [role="radiogroup"] {
        display: flex;
        flex-direction: column;
        gap: 4px; /* 选项之间的间距，可自行调整 */
        padding: 0 4px;
    }

    /* 3. 单个选项基础样式：美化布局，去除默认边距 */
    .stRadio [role="radiogroup"] label {
        padding: 10px 15px;
        border-radius: 6px;
        cursor: pointer;
        transition: all 0.2s ease-in-out; /* 悬浮过渡效果，更流畅 */
        margin: 0; /* 去除默认外边距 */
        color: #333333; /* 默认文字颜色 */
        font-size: 14px;
    }
    ...
    .stRadio [role="radiogroup"] label:hover {
        background-color: #e6f7ff; /* 悬浮背景色（浅蓝色），可自行调整 */
        color: #1890ff; /* 悬浮文字色（深蓝色），可自行调整 */
    }

    /* 5. 选中项样式：突出显示当前页面，便于识别 */
...
# ---------------------- 侧边栏导航（无圆圈+悬浮变色） ----------------------
st.sidebar.title("📑 缝合页面")
page = st.sidebar.radio(
    "导航标签",  # 标签将被隐藏，仅作为占位
    ["主页", "个人简历生成器", "动物图鉴", "南宁美食数据", "数字档案"],
    index=0,  # 默认选中主页
    help="点击切换不同功能页面",
    label_visibility="collapsed"  # 隐藏radio的默认标签，仅显示选项文字
)
